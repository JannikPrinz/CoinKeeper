...
void ProfileChooserPresenter::CreateNewProfile()
{
    ...
    if (userClickedOk && !newProfileName.isEmpty()) {
        Database::CreateNewProfile(newProfileName.toStdString());
        RefreshProfilesList();
    }
}
...
void ProfileChooserPresenter::DeleteProfile()
{
    ...
    case QMessageBox::Yes:
        Database::DeleteProfile(currentProfiles[view->GetSelectedRow()].second);
        RefreshProfilesList();
        break;
    case QMessageBox::No:
        break;
...
void ProfileChooserPresenter::ChangeProfileName()
{
    ...
    if (userClickedOk && !newProfileName.isEmpty()) {
        Database::ChangeProfileName(currentProfiles[view->GetSelectedRow()].second, newProfileName.toStdString());
        RefreshProfilesList();
    }
}
